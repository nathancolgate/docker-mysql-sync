...
while ! mysqladmin -h "${SRC_HOST}" -u "${SRC_USER}" --password="${SRC_PASS}" ping; do
...
mysqldump \
  ...
  --password="${SRC_PASS}" \
  --host="${SRC_HOST}" \
  --ignore-table="${SRC_NAME}.versions" \
  --no-tablespaces \
  "${SRC_NAME}" \
  > /sql/dump.sql

while ! mysqladmin -h "${DEST_HOST}" -u "${DEST_USER}" --password="${DEST_PASS}" ping; do
